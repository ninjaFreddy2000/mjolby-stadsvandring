...
import { getSupabase, isConfigured, FUNCTIONS_BASE, PRICES, BILLING_ENABLED, PAYWALL_LINKS, PAYMENT_LINKS, TRIAL_DAYS } from './config.js';
...
// ── Gratis provperiod (7 dagar, friktionsfri) ────────────────────────────────
// Lagras som starttidstämpel (ms) i localStorage. Ett tryck startar perioden;
// hasAccess() släpper igenom så länge den är aktiv. Kan bara startas en gång.
const TRIAL_KEY = 'sv_trial_start';
const TRIAL_MS = (TRIAL_DAYS || 7) * 24 * 60 * 60 * 1000;
function trialStartTs() { try { return parseInt(localStorage.getItem(TRIAL_KEY) || '', 10) || 0; } catch (e) { return 0; } }
function trialUsed() { return trialStartTs() > 0; }
function trialMsLeft() { const s = trialStartTs(); return s ? Math.max(0, s + TRIAL_MS - Date.now()) : 0; }
function trialActive() { return trialMsLeft() > 0; }
function trialEnded() { return trialUsed() && !trialActive(); }
function trialDaysLeft() { return Math.ceil(trialMsLeft() / (24 * 60 * 60 * 1000)); }
function startTrial() {
  if (trialUsed()) return;
  try { localStorage.setItem(TRIAL_KEY, String(Date.now())); } catch (e) {}
  state.trial = true; state.ready = true;
  if (ctx && typeof ctx.track === 'function') { try { ctx.track('trial_start', { days: TRIAL_DAYS }); } catch (e) {} }
  emit();
}

let ctx = null;
let supa = null;
const state = { stadsjakt: false, trial: false, cities: new Set(), ready: false };
const listeners = [];
...
export function hasAccess(city) {
  if (PAYWALL_LINKS) return softUnlocked() || trialActive();   // lätt paywall: köpt (localStorage) eller pågående provperiod
...
export async function initBilling(context) {
  ctx = context;
  if (PAYWALL_LINKS) { state.stadsjakt = softUnlocked(); state.trial = trialActive(); state.ready = true; emit(); return; }
...
export function openPaywall(city, cityName) {
  ...
  const canTrial = PAYWALL_LINKS && !trialUsed();     // provperiod ej påbörjad
  const trialIsOver = PAYWALL_LINKS && trialEnded();  // provperiod slut → nudge till köp
  card.innerHTML = `
    ...
    ${trialIsOver ? `<p class="pw-sub" style="color:var(--accent,#0A2A6B);font-weight:600;margin-top:-.6em">${en()
      ? `Your ${TRIAL_DAYS}-day free trial has ended. Unlock to keep exploring.`
      : `Din ${TRIAL_DAYS}-dagars provperiod är slut. Lås upp för att fortsätta upptäcka.`}</p>` : ''}

    ${canTrial ? `<div class="pw-opt feat">
      <div class="pw-row"><b>🎁 ${en() ? `Try free for ${TRIAL_DAYS} days` : `Prova gratis i ${TRIAL_DAYS} dagar`}</b></div>
      <small>${en() ? 'Full access to every guided walk. No account, no card — start instantly.' : 'Full tillgång till alla guidade vandringar. Inget konto, inget kort — börja direkt.'}</small>
      <button class="cta" id="pw-trial">${en() ? 'Start free trial' : 'Starta gratis provperiod'}</button>
    </div>` : ''}

    <div class="pw-opt${canTrial ? '' : ' feat'}">
      <div class="pw-row">
  ...
  const trialBtn = card.querySelector('#pw-trial');
  if (trialBtn) trialBtn.onclick = () => {
    startTrial();
    o.setAttribute('aria-hidden', 'true');
    toast(en() ? `🎉 Trial started — ${TRIAL_DAYS} days free!` : `🎉 Provperiod igång — ${TRIAL_DAYS} dagar gratis!`);
  };
  if (PAYWALL_LINKS) {
...
export function mountBillingProfile(el, opts = {}) {
  ...
  function renderLinks() {
    ...
    const price = en() ? PRICES.stadsjakt.label_en : PRICES.stadsjakt.label_sv;
    const buyCta = (cls, id) => `<button class="${cls}" id="${id}">🗝️ ${en() ? 'Get Stadsjakten — ' : 'Skaffa Stadsjakten — '}${price}</button>`;
    let badge, body;
    if (unlocked) {
      badge = '🗝️ ' + (en() ? 'Unlocked' : 'Upplåst');
      body = `<p class="fb-sub" style="margin:.4em 0 0">${en() ? 'All guided walks unlocked. Thank you for your support!' : 'Alla guidade vandringar upplåsta. Tack för ditt stöd! 💛'}</p>`;
    } else if (trialActive()) {
      const d = trialDaysLeft();
      badge = '🎁 ' + (en() ? 'Trial' : 'Provperiod');
      body = `<p class="fb-sub" style="margin:.4em 0 0">${en()
        ? `Free trial — ${d} ${d === 1 ? 'day' : 'days'} left. Every guided walk is open.`
        : `Provperiod — ${d} ${d === 1 ? 'dag' : 'dagar'} kvar. Alla guidade vandringar är öppna.`}</p>
        <div class="acc-actions" style="margin-top:10px">${buyCta('cta', 'bp-buy')}</div>`;
    } else if (trialEnded()) {
      badge = en() ? 'Free' : 'Gratis';
      body = `<p class="fb-sub" style="margin:.4em 0 0">${en() ? 'Your free trial has ended.' : 'Din provperiod är slut.'}</p>
        <div class="acc-actions" style="margin-top:10px">${buyCta('cta', 'bp-buy')}</div>`;
    } else {
      badge = en() ? 'Free' : 'Gratis';
      body = `<div class="acc-actions" style="margin-top:10px">
        <button class="cta" id="bp-trial">🎁 ${en() ? `Try free for ${TRIAL_DAYS} days` : `Prova gratis i ${TRIAL_DAYS} dagar`}</button>
        ${buyCta('om-link', 'bp-buy')}
      </div>`;
    }
    wrap.innerHTML = `
    ...
    const tr = wrap.querySelector('#bp-trial'); if (tr) tr.onclick = () => {
      startTrial();
      toast(en() ? `🎉 Trial started — ${TRIAL_DAYS} days free!` : `🎉 Provperiod igång — ${TRIAL_DAYS} dagar gratis!`);
    };
  }
